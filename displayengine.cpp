 ...
 */
static int open_display_engine(const struct hw_module_t* module, const char* name, struct hw_device_t** device);
...
static void update_tvout_scale_mode(struct de_context_t *ctx) {
    ...
    int retval;
    int de_support_2x;
    // we should read from de
    if (!ctx) {
        return;
    }

    if (ctx->vtv_ui_height*2 <= ctx->tvout_height && ctx->vtv_ui_width*2 <= ctx->tvout_width) {
        use2x_scale = 1;
    } else {
        use2x_scale = 0;
    }
...
static void update_virtual_tv_config(struct de_context_t *ctx, int *vtv_width, int *vtv_height, int *tv_width,
    ...
    // try put virtual ui resolution into system.prop
    ...
    if (ioctl(ctx->mFD, DEIO_GET_MDSB_INFO, &info) < 0) {
        ...
        if (info.drop_line_front >= 0 && info.drop_line_front < h && info.drop_line_back >= 0 && info.drop_line_back < h) {
...
static int de_set_displayer(struct de_control_device_t *dev, displayer_config *displayer_config) {
    int status = -EINVAL;
    unsigned int mask;
    ...
    if (mask & displayer_config->displayer_id) {
        ...
        // muse called after update_virtual_tv_config
...
int de_get_tv_display_scale(struct de_control_device_t *dev, int *pxscale, int *pyscale) {
    int de_param;
    struct de_context_t* ctx = (struct de_context_t*) dev;
...
static void de_set_display_mode(struct de_control_device_t *dev, int mode) {
    ...
    case DISPLAY_MODE_MEDIUM_BANDWIDTH:
        de_set_display_mode_single(dev, MODE_DISP_DOUBLE_DEFAULT_NO_SYNC_TV_GV_LCD_G);
        break;
    case DISPLAY_MODE_HIGH_BANDWIDTH:
        de_set_display_mode_single(dev, MODE_DISP_DOUBLE_DEFAULT_SYNC_EXT_TV_GV_LCD_GV);
        break;
    default:
